...
public class MemoController {
	...
	@GetMapping("/edit/{id}")
	public ModelAndView edit(@PathVariable(name = "id") Long id, ModelAndView model) {
	    MemoBook MemoBook = MemoBookService.get(id);
	    model.addObject("MemoBook", MemoBook); // memoBookを追加する
	    model.setViewName("edit"); // ビューの設定
	    return model;
	}

	@PostMapping("/edit/{id}")
	public String edit(@PathVariable(name = "id") Long id, @ModelAttribute MemoBook MemoBook, RedirectAttributes redirectAttributes) {
	    MemoBook.setId(id); 
	    MemoBookService.save(MemoBook); 

	    // 成功時のフラッシュメッセージを追加
	    redirectAttributes.addFlashAttribute("successMessage", "メモが正常に編集されました。");
